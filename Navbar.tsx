...
const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate, cartCount, showCart, isDarkMode, toggleTheme }) => {
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-12">
          ...
          <div className="flex items-center gap-4 border-l border-black/10 dark:border-white/10 pl-8 ml-2">
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 text-black/60 dark:text-white/60 hover:text-primary dark:hover:text-primary transition-colors"
        ...
        <div className="flex md:hidden items-center gap-4">
          <button
            onClick={toggleTheme}
            className="p-1 text-black/60 dark:text-white/60 hover:text-primary"
            aria-label="Toggle Theme"
          >
            <span className="material-icons">{isDarkMode ? 'light_mode' : 'dark_mode'}</span>
          </button>
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`text-xl font-semibold text-left transition-colors ${
                  currentPage === link.value ? 'text-primary' : 'text-black/60 dark:text-white/60 hover:text-primary dark:hover:text-primary'
                }`}
              >
                {link.label}
              </button>
